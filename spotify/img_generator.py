"""وحدة إنشاء صور بطاقات الطلاب PNG - Penn State LionPATH"""
import random
...
def generate_html(first_name, last_name, school_id='2565'):
    """
    إنشاء محتوى HTML لـ Penn State LionPATH

    Args:
        first_name: الاسم الأول
        last_name: اسم العائلة
        school_id: معرّف المدرسة (ID)
...
<head>
    ...
    <style>
        ...
        body {{
            ...
            background-color: #e0e0e0; /* خلفية المتصفح */
            margin: 0;
            padding: 20px;
        ...
        /* شريط التنقل العلوي لـ LionPATH */
        .header {{
...
def generate_image(first_name, last_name, school_id='2565'):
    """
    إنشاء لقطة شاشة لـ Penn State LionPATH بصيغة PNG
    ...
    try:
        ...
        # استخدام Playwright لالتقاط الشاشة (بدلاً من Selenium)
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            page = browser.new_page(viewport={'width': 1200, 'height': 900})
            page.set_content(html_content, wait_until='load')
            page.wait_for_timeout(500)  # انتظار تحميل الأنماط
        ...
        raise Exception("يجب تثبيت playwright عبر: pip install playwright && playwright install chromium")
...
if __name__ == '__main__':
    ...
    # إصلاح مشكلة تشفير وحدة تحكم (Console) Windows
    if sys.platform == 'win32':
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

    print("اختبار إنشاء صورة PSU...")
    ...
    try:
        ...
        print(f"✓ تم إنشاء الصورة بنجاح! الحجم: {len(img_data)} بايت")
        print("✓ تم الحفظ باسم test_psu_card.png")
